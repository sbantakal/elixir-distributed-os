defmodule NodeServer do
  ...
  def initiateGossip(numNodes, startTime) do
    ...
    nodename = findrandomalivenode(Enum.to_list(1..numNodes),offlineNodes)
    GenServer.cast(nodename, {:receive, numNodes, startTime, nodename})
    NodeServer.waitfunc(startTime)
  end

  def findrandomalivenode(noderange, offlineNodes) do
    randNode = Enum.random(noderange)
    nodename = String.to_atom("Child_"<>Integer.to_string(randNode))
    if Map.has_key?(offlineNodes,nodename) == :false do
      nodename
    else
      findrandomalivenode(List.delete(noderange, nodename), offlineNodes)
    end
  end
  ...
  def disconnectNode(nodename, numNodes, startTime) do
  ...
  def getRandAliveNeighbour(neighList) do
    if Enum.empty?(neighList) do
      ...
      if Map.has_key?(offlineNodes, nodeName) do
  ...
  def handle_cast({:send_message, numNodes, startTime}, node_status) do
    ...
    if (destNode != :false) do
      ...
      spawn_link( fn -> GenServer.cast(destNode, {:receive, numNodes, startTime, destNode}) end)
      GenServer.cast(self(), {:send_message, numNodes, startTime})
    end
    {:noreply,node_status}
  end
