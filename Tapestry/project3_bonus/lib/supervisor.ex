defmodule NodeSupervisor do
  ...
  def hashfunction(numOfNodes) do
    ...
    IO.puts "**********************************************************"
    IO.puts "Identifier Length => #{level}"
  ...
  def failnodes(numNodes, failpercent) do
    failcount = ceil(numNodes * (failpercent/100))
    IO.puts "No of nodes marked Failed => #{failcount}"
    [{_, allnodes}] = :ets.lookup(:table,"Tapestry Nodes")

    failnodes = Enum.take_random(Map.keys(allnodes), failcount)
    Enum.each(failnodes, fn x ->
      NodeServer.failstatus(x)
    end)
  end
end
