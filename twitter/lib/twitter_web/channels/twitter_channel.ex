defmodule TwitterWeb.TwitterChannel do
  ...
  import Comeonin.Argon2, only: [checkpw: 2, dummy_checkpw: 0, hashpwsalt: 1]
  ...
  def handle_in("login", %{"username" => uname, "password" => password}, socket) do
    ...
    result =
      if user_exists == [] do
        ...
        if Comeonin.Argon2.checkpw(password, pass) do
          uname
        else
          "Incorrect Username or Password"
        end
  ...
  def handle_in(
    ...
    hashed_pass = Comeonin.Argon2.hashpwsalt(password)

    if(user_exists == []) do
      :ets.insert_new(:user, {uname, hashed_pass, email})
  ...
  defp sendTweet2Follower(tweeter, follower, tweet, tweetId) do
    ...
    # assigntweets(tweeter <> " tweeted this", follower, tweetId)
